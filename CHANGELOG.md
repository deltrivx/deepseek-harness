...
- **The icon rail's rows are full-width tap targets.** They had only been given a minimum height; their width was still squeezed by the rail's own padding, leaving 36 px wide strips in a 56 px rail. The rail's padding is now tighter when collapsed and the rows carry a minimum width, so all six entries measure at least 44 px in both directions.
- **Scoped the rail padding rule correctly.** The sidebar content root sits one wrapper element below the sidebar column, so a child combinator never matched it; the rule now uses a descendant selector.
