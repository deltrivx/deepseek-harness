...
- **The mobile drawer no longer draws itself while the sidebar is closed.** The drawer's geometry rules had no open/closed condition, so a full-height panel kept covering the conversation even with the sidebar collapsed — a ghost icon rail sat on top of the content and would not go away. The rules are now scoped to the expanded state (the frame element drops its collapsed attribute when open) and the collapsed column is removed from the layout entirely.
