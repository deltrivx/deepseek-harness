...
- **背景开关**、**整体 / 侧栏 / 次级不透明度**、**毛玻璃模糊**、**壁纸压暗**、**填充方式**、**位置**
- 高级项还包含配置页底色、输入框底色等
...
```css
/* 例：让所有面板更透一点，并给侧边栏单独再压暗一层 */
:root { --dsw-bgw-layer1: #ffffff; }
[class*="sidebar"] { background-color: rgba(0, 0, 0, 0.25) !important; }
```

若希望**完全接管**而不是追加，用 `DSH_BACKGROUND_CSS` 指定另一个文件路径，此时默认样式不再生成。
