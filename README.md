...
> 配置文件与凭据保存在 `/root/.dsh` 挂载目录中。容器部署不依赖宿主机的 `file://` 打开能力；如果浏览器提示无法打开本地配置文件，请使用 WebUI 的设置页面重新加载，或直接编辑持久化目录中的配置文件后重启容器。不要在 Unraid 模板中覆盖 `/entrypoint.sh` 或 `/app/proxy.cjs`，否则旧的启动脚本可能覆盖镜像内已修复的代理。
